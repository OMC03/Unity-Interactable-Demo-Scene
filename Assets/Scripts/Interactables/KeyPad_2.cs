...
public class KeyPad_2 : Interactable
{
    public KeyCard card;
    ...
    void Update()
    {
        ...
        else if (anim.GetCurrentAnimatorStateInfo(0).IsName("Closed"))
        {
            ...
            if (!keyPad.doorOpen && !doorUnlocked) // Check if door is not open and not already unlocked
            {
                keyPad.UnlockKeyPad2(true);  // Synchronize with the main keypad
    ...
    public void UnlockKeyPad(bool unLocked)
    {
    ...
    protected override void Interact()
    {
        ...
        {
            Debug.LogError("KeyCard component is not assigned to the KeyPad_2 script.");
            return;
        }
        ...
        if (!isLocked)
        {
            ...
            keyPad.doorOpen = doorOpen; // Synchronize the state with the main keypad
